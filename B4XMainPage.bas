...
Private Sub B4XPage_Created (Root1 As B4XView)
	...
	Financiera.Initialize
	B4XPages.AddPage("Financiera", Financiera)
	Energia.Initialize
	...
	Grafica.Initialize
	B4XPages.AddPage("Grafica", Grafica)
	Calcular.Initialize
...
Public Sub Leer_Usuarios()
	...
	If Main.mSQL.IsInitialized Then
		mRS = Main.mSQL.ExecQuery("SELECT usu_nombre FROM sie_usuarios")
		Main.mULT_USUARIO = -1
		...
		Loop
		Main.mULT_USUARIO = -1
		mRS = Main.mSQL.ExecQuery("SELECT usu_clave FROM sie_usuarios")
		Do While mRS.NextRow
			Main.mULT_USUARIO = Main.mULT_USUARIO + 1
			Main.mCLAVES(Main.mULT_USUARIO) = mRS.GetString("usu_clave")
		Loop
		Main.mULT_USUARIO = -1
		mRS = Main.mSQL.ExecQuery("SELECT usu_derecho FROM sie_usuarios")
		Do While mRS.NextRow
			Main.mULT_USUARIO = Main.mULT_USUARIO + 1
	...
	For x = 0 To Main.mULT_USUARIO
		If Main.mUSUARIO = Main.mUSUARIOS(x) Then
			...
			If Main.mCLAVE = Main.mCLAVES(x) Then
				...
				Log(Main.mUSUARIO & " - " & Main.mCLAVE & " - " & Main.mDERECHO)
