...
Private Sub B4XPage_Created (Root1 As B4XView)
	...
	TBL_Calcular.ColsWidth=Array As Int(80dip, 320dip, 80dip, 80dip, 80dip, 80dip, 80dip)
	TBL_Calcular.ColsType=Array As Int(TBL_Calcular.TypeString, TBL_Calcular.TypeString, TBL_Calcular.TypeString)
	TBL_Calcular.ColsAlignment=Array As String("CENTER", "CENTER", "CENTER", "CENTER", "CENTER", "CENTER", "CENTER")
...
'	mSQL.Initialize2("net.sourceforge.jtds.jdbc.Driver", $"jdbc:jtds:sqlserver://${mSERVIDOR};DatabaseName=SIE;"$, mADMINISTRADOR, mPASSWORD)
	...
	If Not(mCOMPLETOS) Then
		L_Estado_1.Visible = True
		L_Estado_1.Text = "Falta información para poder determinar los Indicadores de Efectividad"
	End If
	If mCOMPLETOS Then
		mIND_CUMPLE = ""
		x = mULT_CALCULAR
		...
		For x = 0 To mULT_CALCULAR
			...
			If TBL_Calcular.GetCellValue(x, 0) = "ECOB" Then
				If mFAC <> 0 Then
