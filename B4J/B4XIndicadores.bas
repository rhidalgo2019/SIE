...
Private Sub B4XPage_Created (Root1 As B4XView)
	...
	TBL_Indicadores.ColsWidth=Array As Int(400dip, 80dip, 80dip, 80dip, 80dip, 80dip)
	...
	TBL_Indicadores.ColsAlignment=Array As String("CENTER", "CENTER", "CENTER", "CENTER", "CENTER", "CENTER")
...
Sub Cargar_Valores()
	...
	If Main.mSQL.IsInitialized Then
		...
		Do While mRS.NextRow
			...
			End Select
			If mVALORES(Main.mULT_INDICADOR) = "" Then
				mVALORES(Main.mULT_INDICADOR) = "0"
			End If
			If mCUMPLIMIENTOS(Main.mULT_INDICADOR) = "" Then
				mCUMPLIMIENTOS(Main.mULT_INDICADOR) = "0"
			End If
		Loop
...
'			Log(mCLAVES(X) & " - " & mDESCRIPCIONES(X) & " - " & mEXTRAIDOS(X))
		...
		Next
		' Actualizar colores
		For x = 0 To TBL_Indicadores.RowCount -1
			mTMP_STR_1 = Main.mFORMATO(TBL_Indicadores.GetCellValue(x, 1), "T")
			mTMP_STR_2 = Main.mFORMATO(TBL_Indicadores.GetCellValue(x, 2), "T")
			mTMP_FLOAT = (mTMP_STR_1 / mTMP_STR_2) * 100
			mVALOR = Main.mFORMATO(mTMP_FLOAT.As(String), "P")
			TBL_Indicadores.SetCellValue(x, 3, mVALOR)
			'
			mTMP_STR_1 = Main.mFORMATO(TBL_Indicadores.GetCellValue(x, 1), "T")
			mTMP_STR_2 = Main.mFORMATO(TBL_Indicadores.GetCellValue(x, 2), "T")
			mTMP_STR_3 = Main.mFORMATO(TBL_Indicadores.GetCellValue(x, 4), "T")
			If mTMP_STR_1 < mTMP_STR_3 Then
				TBL_Indicadores.SetRowCustomize(x, xui.Color_RGB(0, 0, 255), xui.Color_RGB(255, 255, 0), xui.CreateDefaultFont(12))
			End If
			If mTMP_STR_1 > mTMP_STR_3 And mTMP_STR_1 < mTMP_STR_2 Then
				TBL_Indicadores.SetrowCustomize(x, xui.Color_RGB(0, 0, 255), xui.Color_RGB(70, 130, 180), xui.CreateDefaultFont(12))
			End If
			If mTMP_STR_1 >= mTMP_STR_2 Then
				TBL_Indicadores.SetrowCustomize(x, xui.Color_RGB(0, 0, 255), xui.Color_RGB(0, 255, 255), xui.CreateDefaultFont(12))
			End If
		Next
