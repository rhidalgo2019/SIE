...
Private Sub B_Guardar_Click
	...
	Dim mRESPUESTA_QUERY As String
	
	mCOMPLETOS = True
	For x = 0 To Main.mULT_EXTRAIDO
		mEXTRAIDOS(x) = Main.mFORMATO(TBL_Pozos.GetCellValue(x, 1), "T")
...
'				mEXTRAIDOS(x) = cINT(TBL_Pozos.GetCellValue(x, 2))
				Select Case Main.mMES
					Case 1
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_ene = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 2
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_feb = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 3
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_mar = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 4
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_abr = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 5
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_may = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 6
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_jun = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 7
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_jul = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 8
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_ago = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 9
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_sep = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 10
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_oct = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 11
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_nov = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
					Case 12
						Main.mSSQL.ExecNonQuery("UPDATE sie_extraido SET ext_dic = " & mEXTRAIDOS(x) & " WHERE ext_clave = '" & mCLAVES(x) & "' AND ext_anio = " & Main.mANIO)
				End Select
