...
Sub Cargar_Valores()
	...
	If mSQL.IsInitialized Then
		mRS = mSQL.ExecQuery("SELECT * FROM sie_valores WHERE val_derecho = 4 AND val_anio = " & ANIO)
		Main.mULT_DATOS = -1
...
Private Sub TBL_Contabilidad_CellClick(Row As Int, Col As Int)
	...
	If Col >= 1 Then
		mVALOR = mFORMATO(TBL_Contabilidad.GetCellValue(Row, Col), "T").As(Int)
...
Private Sub TBL_Contabilidad_Modified(Row As Int, Col As Int)
	...
	If Row < 2 Then
		mCADENA = mFORMATO(TBL_Contabilidad.GetCellValue(Row, Col), "M")
	Else
		mCADENA = mFORMATO(TBL_Contabilidad.GetCellValue(Row, Col), "N")
	End If
...
Private Sub B_Guardar_Click
	...
	For x = 0 To Main.mULT_DATOS
		...
		mEXTRAIDOS(x) = mFORMATO(TBL_Contabilidad.GetCellValue(x, 1), "T")
		...
		If mSQL.IsInitialized  Then
			For x = 0 To Main.mULT_DATOS
				...
				Select Case Main.mMES
					Case 1
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_ene = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 2
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_feb = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 3
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_mar = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 4
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_abr = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 5
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_may = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 6
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_jun = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 7
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_jul = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 8
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_ago = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 9
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_sep = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 10
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_oct = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 11
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_nov = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
					Case 12
						mSQL.ExecNonQuery("UPDATE sie_valores SET val_dic = '" & mEXTRAIDOS(x) & "' WHERE val_anio = " & ANIO & " AND val_descripcion = '" & mDATOS(x) & "'")
				End Select
